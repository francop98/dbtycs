...
document.addEventListener('DOMContentLoaded', () => {
  ...
  onAuthStateChanged(auth, (user) => {
    if (user) {
      currentUser = user;
      precargarPerfilExistente(user.uid);
    } else {
      precargarPerfilExistente('invitado');
    }
  });
...
function guardarPerfil() {
  ...
  const getVal = (id) => document.getElementById(id)?.value?.trim() || '';
  const userId = currentUser ? currentUser.uid : 'invitado';
  const storageKey = `dbtycs_perfil_${userId}`;

  const perfilPrevio = JSON.parse(localStorage.getItem(storageKey) || '{}');
  const perfil = {
  ...
  localStorage.setItem(storageKey, JSON.stringify(perfil));
  localStorage.setItem('dbtycs_perfil', JSON.stringify(perfil));
}

function precargarPerfilExistente(userId) {
  const storageKey = `dbtycs_perfil_${userId}`;
  const guardado = localStorage.getItem(storageKey) || localStorage.getItem('dbtycs_perfil');
  if (!guardado) return;
