...
import { guardarPerfilEnNube, sincronizarPerfil } from './firestore-sync.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      // Si hay perfil en la nube, lo bajamos y precargamos con ese.
      // Si no, sincronizarPerfil sube el local (si existe) como primera migración.
      const perfilSincronizado = await sincronizarPerfil();
      precargarPerfilExistente(perfilSincronizado);
    } else {
...
async function guardarPerfil() {
  ...
  const perfilPrevio = JSON.parse(localStorage.getItem('dbtycs_perfil') || '{}');
  const perfil = {
  ...
  localStorage.setItem('dbtycs_perfil', JSON.stringify(perfil));

  // Si hay sesión iniciada, el perfil también se guarda en la nube
  if (currentUser) {
    try {
      await guardarPerfilEnNube(perfil);
    } catch (e) {
...
function precargarPerfilExistente(perfilForzado) {
  const guardado = perfilForzado ? JSON.stringify(perfilForzado) : localStorage.getItem('dbtycs_perfil');
  if (!guardado) return;

  try {
    const perfil = typeof guardado === 'string' ? JSON.parse(guardado) : guardado;
